...
class GenerateBacktestReportView(APIView, GenerateReportMixin):
    ...
    def validate_request_data(data):
        if "results" not in data:
            return Response(
                {"error": "'results' key is missing in the request data."}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def validate_backtest_data(backtest_data):
        ...
        if incorrect_type_fields:
            return Response(
                {"error": f"Incorrect data types for fields: {', '.join(incorrect_type_fields)}"},
    ...
    def get_historical_data(symbol):
        try:
            ...
            historical_data = StockHistoryData.objects.filter(
                symbol=symbol.upper(), timestamp__gte=timestamp_30_days_ago
            ).order_by("timestamp")

            return [
                {"timestamp": datetime.fromtimestamp(data.timestamp).strftime("%Y-%m-%d"), "price": data.open_price}
