...
def predict_single_feature(model, future_df, feature_name):
    ...
    return (
        forecast[[DATE_COLUMN, "yhat"]]
        .tail(PREDICTION_PERIOD)
        .rename(columns={"yhat": feature_name})
...
def predict_all_features(symbol):
    ...
    return {feature: predict_single_feature(model, future_df, feature) for feature, model in models.items()}
    ...
    with transaction.atomic():
        if predicted_data_list:
            PredictedStockData.objects.bulk_update(
                predicted_data_list, fields=["predicted_" + col for col in ALL_COLUMNS] + ["created_at"]
            )
