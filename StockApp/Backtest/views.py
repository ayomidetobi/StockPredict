    ...
    def validate_backtest_input(self, data):
        ...
        for field in required_fields:
            if field not in data or not data[field]:
                return Response(
                    {"error": f"{field} is required and cannot be empty"}, status=status.HTTP_400_BAD_REQUEST
                )
    ...
    def post(self, request):
        try:
            ...
            try:
                ...
                return Response({"error": f"Invalid stock symbol: {str(e)}"}, status=status.HTTP_400_BAD_REQUEST)
            except Exception as e:
                logger.error(f"Error fetching stock prices: {str(e)}")
                return Response({"error": "Unable to fetch stock data"}, status=status.HTTP_503_SERVICE_UNAVAILABLE)
            if not stock_prices:
            ...
            try:
                ...
                return Response(
                    {"error": "Failed to generate backtest report"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
